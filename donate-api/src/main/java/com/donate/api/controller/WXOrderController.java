...
public class WXOrderController {
    ...
    public void getJSSDKCallbackData(HttpServletRequest request,
        ...
        try {
            synchronized (this) {
                ...
                Map donateOrder = wxOrderService.getDonateOrder(payNotifyResult.getOutTradeNo());
                //查询订单状态，已成功的，无需再进行处理
                if (donateOrder != null && donateOrder.get("status").toString().equals(PayStatusEnum.SUCCESS.getValue().intValue())) {
    ...
    private ServiceResult checkOrder(Map donateOrder, WxPayOrderNotifyResult orderNotifyResult) {
        ServiceResult result = new ServiceResult();
        if (donateOrder == null) {
            return result.failed("out_trade_no not matching");
        }
        BigDecimal cardinal = new BigDecimal(100);
        if(donateOrder.get("amount")==null){
			return result.failed("payAmount no matching");
		}

        BigDecimal donateAmout = new BigDecimal(Double.valueOf(donateOrder.get("amount").toString()));
        if (donateAmout.multiply(cardinal).intValue() != orderNotifyResult.getTotalFee().intValue()) {
